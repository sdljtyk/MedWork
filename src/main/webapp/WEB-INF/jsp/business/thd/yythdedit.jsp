...
<script type="text/javascript">
	...
	var yythdmxdelete = function() {
		_confirm('您确定要删除选择的退货药品吗?', null, function() {
			...
			if (rows.length > 0) {
				...
				$("#yythdmxForm").attr("action",
						'${baseurl}/thd/yythdmxdelete.action');
	...
	function yythdmxsave() {
		_confirm("您确定要保存选择的退货药品信息吗？", null, function() {
			...
			if (rows.length > 0) {
				$("#indexs").val(indexs.join(','));
				$("#yythdmxForm").attr("action",
						'${baseurl}/thd/yythdmxsave.action');
	...
	var columns = [ [
			...
			},
			{
				...
				width : 90,
				formatter : function(value, row, index) {
					return '<input type="text" name="yythdmxs[' + index
							+ '].backreason" value="' + (value ? value : "")
							+ '" />';
				}
			},
			{
...
<BODY>
						...
						<TABLE class="toptable grid" border=1 cellSpacing=1 cellPadding=4
							...
							<TBODY>
								...
								<TR>
									...
									<TD class=category width="35%">${yythd.backnumber}</TD>
								...
								<TR>
									...
									<TD class=category width="35%"><input type="text"
										name="yylxr" id="yylxr" value="${yythd.yylxr}"
								...
								<TR>
									...
									<TD colspan=3><textarea rows="2" cols="30"
											name="backother">${yythd.backother}</textarea></TD>
								</TR>

								<tr>
									<td colspan=4 align=center class=category><a href="#"
										onclick="yythdsave()" class="easyui-linkbutton"
										iconCls='icon-save'>保存</a> <a href="#" onclick="yythdsubmit()"
	...
	<form id="yythdmxForm" name="yythdmxForm" method="post">
		<input type="hidden" name="indexs" id="indexs" /> <input
			type="hidden" name="id" value="${yythd.id}" />
